    ...
    return (
        <DefaultLayout>
            <section className="flex sm items-center justify-center">
                <div
                    className="inline-block max-w-lg text-center justify-center bg-warning-100 p-2 border-warning-100 shadow rounded-md transition duration-500 ease-in-out border hover:border-warning-200 text-sm">
                    Looking for the OG SSingh.Net?<Link isExternal className={buttonStyles({
                    size: "sm",
                    variant: "flat",
                    radius: "sm",
                    color: "warning"
                }) + " ml-2"} href="https://og.ssingh.net/">Click Here</Link>
                </div>
            </section>

            
            <section
                className="flex flex-col items-center justify-center gap-4 m-5 py-8 md:py-10 border-2 shadow-md rounded-md transition duration-500 ease-in-out hover:border-blue-400">
                ...
                <div
                    className="gap-5 mt-5 mx-5 md:m-0 inline-block max-w-85 text-center justify-center transition duration-300 ease-in-out hover:scale-105">
                    <Card
                        radius="sm"
                        className="border-1 border-danger-200 p-3 px-5 text-small"
                        isBlurred
                    >
                        <i>One has no right to love or hate anything if one has not acquired a thorough knowledge of its
                            nature.</i>
            ...
            <section className="my-4 group w-full">
                <h1 className="text-2xl w-full font-bold underline">Certifications:</h1>
            ...
            {siteConfig.certificates && siteConfig.certificates.length > 0 ? (
                <div className="m-5 flex justify-center">
                    <div className="w-full max-w-3xl">
                        <CertificateCard
                            compact
                            name={siteConfig.certificates[0].name}
                            provider={siteConfig.certificates[0].provider}
                            link={siteConfig.certificates[0].link}
                            issueDate={siteConfig.certificates[0].issueDate}
                            expiryDate={siteConfig.certificates[0].expiryDate}
                            image={siteConfig.certificates[0].image}
                        />
                    </div>
                </div>
                ) : null}
            ...
            <section className="my-4 group w-full">
                <h1 className="text-2xl w-full font-bold underline">Education:</h1>
                {siteConfig.education && siteConfig.education.length > 0 ? (
                    <div className="m-5 flex justify-center">
                        <div className="w-full max-w-3xl">
                            <EducationCard
                                institution={siteConfig.education[0].institution}
                                location={siteConfig.education[0].location}
                                degree={siteConfig.education[0].degree}
                                dates={siteConfig.education[0].dates}
                            />
                        </div>
                    </div>
                ) : null}
            ...
            */}
            ...
            <section className="my-4 group w-full">
                ...
                <div className="m-5 grid grid-cols-1 md:grid-cols-2 gap-4">
                    {siteConfig.projects.personal.map((proj: ProjectProps) => (
                        <ProjectCard
                            title={proj.title}
                            subtitle={proj.subtitle}
                            desc={proj.desc}
            ...
            <section className="group flex items-center justify-center w-full">
                <div
                    className="inline-block rounded-md w-full my-0 text-center justify-center border-none outline-none">
                    ...
                    <Card className="pt-4 bg-opacity-0 rounded-md border-none outline-none shadow-none">
                        <div
                            className="m-1 md:m-5 grid grid-cols-3 sm:grid-cols-5 md:grid-cols-6 lg:grid-cols-9 xl:grid-cols-10 gap-4 justify-center">
